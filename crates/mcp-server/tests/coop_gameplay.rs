...
struct FakeGateway { requests: Vec<GatewayRequest>, responses: VecDeque<Result<GatewayResponse, GatewayError>> }

impl GatewayAdapter for FakeGateway {
    fn forward(&mut self, request: GatewayRequest) -> Result<GatewayResponse, GatewayError> {
        self.requests.push(request);
        self.responses.pop_front().unwrap_or(Err(GatewayError::Unavailable))
    }
}
...
fn response() -> JsonValue {
    JsonValue::object([
        ...
        (String::from("players"), JsonValue::Array(vec![
            JsonValue::object([(String::from("peer_id"), JsonValue::string("local-1")), (String::from("role"), JsonValue::string("local"))]),
            JsonValue::object([(String::from("peer_id"), JsonValue::string("ally-1")), (String::from("role"), JsonValue::string("ally"))]),
        ])),
        ...
        (String::from("synchronization"), JsonValue::object([
...
fn call(arguments: &str) -> String {
    format!("{{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",\"params\":{{\"name\":\"{COOP_SYNCHRONIZATION_TOOL}\",\"arguments\":{{{arguments}}}}}}}")
}

#[test]
fn co_op_sync_is_read_only_and_preserves_identity() {
    let gateway = FakeGateway { requests: Vec::new(), responses: VecDeque::from([Ok(GatewayResponse { status: 200, body: response() })]) };
    let mut server = McpServer::with_catalog(gateway, ToolCatalog::coop_gameplay());
    let frame = call("\"instance_id\":\"instance-1\",\"mcp_session_id\":\"session-1\",\"lease_id\":\"lease-1\",\"lease_epoch\":1,\"generation\":4");
    ...
    assert_eq!(server.gateway().requests[0].path, "/v1/instances/instance-1/coop/synchronization");
}

#[test]
fn co_op_sync_rejects_extra_input_before_gateway() {
    let gateway = FakeGateway { requests: Vec::new(), responses: VecDeque::new() };
    let mut server = McpServer::with_catalog(gateway, ToolCatalog::coop_gameplay());
    let frame = call("\"instance_id\":\"instance-1\",\"mcp_session_id\":\"session-1\",\"lease_id\":\"lease-1\",\"lease_epoch\":1,\"generation\":4,\"action\":\"blocked\"");
