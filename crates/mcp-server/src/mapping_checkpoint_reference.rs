...
use crate::catalog::CHECKPOINT_REFERENCE_TOOL;
use crate::checkpoint_reference::{CheckpointReferenceContext, project};
...
use super::{gateway_error_result, has_only_arguments, headers, invalid_params};

const REFERENCE_ARGUMENTS: [&str; 5] = [
    "instance_id",
    "mcp_session_id",
    "lease_id",
    "lease_epoch",
...
pub(super) fn tools_call<G: GatewayAdapter>(
    ...
    if tool_name != CHECKPOINT_REFERENCE_TOOL {
        return invalid_params(request.id, "unknown checkpoint reference tool");
    }
    ...
    reference_call(server, id, arguments, &correlation_id)
}

fn reference_call<G: GatewayAdapter>(
    server: &mut McpServer<G>,
    id: RequestId,
    arguments: &BTreeMap<String, JsonValue>,
    correlation_id: &str,
) -> RpcResponse {
    if !has_only_arguments(arguments, &REFERENCE_ARGUMENTS) {
        return invalid_params(
            id,
            "sts2.checkpoint_reference arguments contain an unsupported field",
        );
    }
    let context = match ReferenceContext::parse(server, arguments, correlation_id) {
        Ok(context) => context,
        Err(message) => return invalid_params(id, message),
    };
    let mut request_headers = headers(&context.mcp_session_id, correlation_id);
    request_headers.insert(
        "x-sts2-caller-id".into(),
        context.projection.caller_id.clone(),
    );
    ...
    match server.gateway.forward(request) {
        Ok(response) => match project(&response.body, &context.projection) {
            ...
            _ => super::tool_result(id, "checkpoint reference unavailable or invalid", true),
        },
...
struct ReferenceContext {
    projection: CheckpointReferenceContext,
...
impl ReferenceContext {
    fn parse<G: GatewayAdapter>(
        ...
        {
            return Err("Checkpoint-reference identity is unsafe or oversized");
        }
        let lease_epoch = bounded_argument(arguments, "lease_epoch")?;
        let caller_id = string_argument(arguments, "caller_id")?;
        if !super::safe_header_value(caller_id) {
            return Err("caller identity is invalid");
        }
        Ok(Self {
            projection: CheckpointReferenceContext {
...
fn string_argument<'a>(
    ...
    arguments
        ...
        .ok_or("Checkpoint-reference identity argument must be a non-empty string")
}
...
fn bounded_argument(
    ...
    match arguments.get(key) {
        Some(JsonValue::Number(value)) if *value >= 0 && *value <= MAX_EPOCH => Ok(*value),
        _ => Err("Checkpoint-reference generation or lease_epoch is outside the protocol bound"),
    }
}
