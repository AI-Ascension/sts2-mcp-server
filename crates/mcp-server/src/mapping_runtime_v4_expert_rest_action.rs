...
use crate::catalog::{EXPERT_REST_ACTION_TOOL, EXPERT_REST_RECONCILE_TOOL, EXPERT_STATE_TOOL};
...
#[path = "mapping_runtime_v4_expert_rest_action_dispatch.rs"]
mod dispatch;
...
pub(super) fn tools_call<G: GatewayAdapter>(
    ...
    if !matches!(
        tool_name,
        EXPERT_STATE_TOOL | EXPERT_REST_ACTION_TOOL | EXPERT_REST_RECONCILE_TOOL
    ) || server.catalog.descriptor(tool_name).is_none()
    {
        return RpcResponse::failure(
            ...
            RpcError::new(
                METHOD_NOT_FOUND,
                "tool is not in the active Runtime-v4 REST-action catalog",
            ),
        );
    }
    ...
    match tool_name {
        ...
        EXPERT_REST_ACTION_TOOL => {
            dispatch::expert_rest_action_call(server, id, arguments, &correlation_id)
        }
        EXPERT_REST_RECONCILE_TOOL => {
            expert_rest_reconcile_call(server, id, arguments, &correlation_id)
        }
        _ => invalid_params(id, "Runtime-v4 REST-action tool is not active"),
    }
}

fn expert_rest_reconcile_call<G: GatewayAdapter>(
    server: &mut McpServer<G>,
    id: RequestId,
    ...
    if !has_only_arguments(arguments, &RECONCILE_ARGUMENTS) {
        return invalid_params(
            id,
            "sts2.expert_rest_reconcile arguments contain an unsupported field",
        );
    }
            ...
            .is_some_and(|expected| expected != mcp_session_id)
    {
        return invalid_params(
            id,
            "expert-rest-reconcile identity is unsafe or not bound to this MCP session",
        );
    }
    ...
    let request = GatewayRequest {
        ...
        path: format!("/v4/instances/{instance_id}/expert-rest-actions/{operation_id}"),
