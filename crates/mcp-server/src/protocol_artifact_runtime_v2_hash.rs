...
pub(crate) fn sha256_hex(bytes: &[u8]) -> String {
    ...
    for chunk in padded.chunks_exact(64) {
        ...
        for index in 16..64 {
            let a = schedule[index - 15].rotate_right(7)
                ^ schedule[index - 15].rotate_right(18)
                ^ (schedule[index - 15] >> 3);
            let b = schedule[index - 2].rotate_right(17)
        ...
        for (target, value) in hash.iter_mut().zip(working) {
...
mod dependency_semantics_tests {
    ...
    #[test]
    fn shared_schema_and_sha256_vectors_reject_semantic_drift() -> Result<(), String> {
        ...
        if sha256_hex(input.as_bytes()) != expected {
